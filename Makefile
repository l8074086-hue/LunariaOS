...
release: $(DISK_IMG) $(BUILD_DIR)/OS.bin
	...
	@tar -czf releases/lunariaos-$(shell git describe --tags --always 2>/dev/null || echo dev).tar.gz -C release disk.img OS.bin run.sh
	@rm -rf release
	@echo "created releases/lunariaos-*.tar.gz"
